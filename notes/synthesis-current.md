...
- **Stage 3 (Power Bond):** unlocks superhuman strength + small Hell conjuring (pulling small items through from Hell). Requires Lucifer to disclose BOTH that he's divine AND his mission (looking for someone) — both pieces required, not either alone. **Elizabeth crossed this threshold at the end of Ch.8** (mission from Lucifer's own mouth, divine nature from Azrael at Lucifer's own telepathic instruction — both count as Lucifer's disclosure) — locked 2026-09-05, see § 12.9. The ability goes unused with her on-page until Ch.11.
...
- **Sophie, James, Claire:** Sophie has not appeared since Ch.5 (near-miss) — not yet properly introduced to Lucifer/Tristan (Ch.13–14 territory). James and Claire are engaged (proposed Ch.4) and hold Power Bonds (know he's divine + his mission) as of Ch.4; Elizabeth learned of their engagement in Ch.7. **James and Claire specifically know Eve's actual name** (Ch.4: "Her name is Eve, but the name itself won't do you any good") — Elizabeth does not (Ch.8 version withholds it). Flag the Director before any scene risks that name reaching Elizabeth ahead of schedule.
...
| James's own ring (missing stone) | Ch.4 — proposes to Claire with it | **Corrected 2026-09-05: NOT repaired in Ch.4.** The missing stone stays missing — this is now a live, unresolved loose end. The Ch.4 "proof of power" moment uses a *separate* ring already on Lucifer's own hand instead (reshaped smaller by strength, given to James) — see world-bible.md "Lucifer's Second Ring." Whoever eventually addresses James's own damaged ring should flag the Director first. | Ch.4 |
| James's wedding ring (separate item) | Referenced Ch.12 (James short on money for it) | Wedding: **August 2nd, the town park** (Ch.27 in the outline — not yet reached in written chapters). Commissioned from Lillith Ch.24, delivered via rose-through-Hell-mechanics Ch.26, physically handed to James Ch.27. None of this has happened on-page yet as of Ch.10. | N/A yet |
...
**Resolved 2026-09-05 (all closed same day the Synthesis run surfaced them):** Ch.10's two Line Editor attribution findings (fixed directly), the Flat-Delivery Exception contradiction on the Ch.9 "or is that also a version of an answer" line (agent file corrected), Ch.17's POV (confirmed staying Elizabeth POV, no restructuring — see § 12.1), the Ch.70+ Lillith-epilogue question (author's explicit call: leave it open/unknown on purpose, no placeholder chapter — see § 12.3), CLAUDE.md's unfilled template (Project Overview section now filled in with real project details — see § 12.4), `story-bible.md` § 19's dangling empty header (deleted — see § 12.6), `unit-plans.md`'s Act Three duplication (redundant draft copy confirmed and deleted — see § 12.7), Ch.4's ability-before-disclosure sequencing bug (reordered, strength-based second-ring mechanism — see `notes/author-questions.md`), and Ch.8's Power Bond timing conflict (Power Bond, not just Travel Bond, actually formed at the end of Ch.8 — see § 12.9).
...
### 12.9 Ch.8 already satisfied Power Bond's conditions for Elizabeth — RESOLVED 2026-09-05
Found during a targeted re-check of this synthesis document against `bond_system_reference.md` and the shipped chapter text (author request: "look for similar problems that conflict my bible," following the Ch.4 sequencing fix). `bond_system_reference.md`'s Stage 3 condition, read literally, is satisfied entirely within Ch.8: Lucifer discloses the mission himself at the park bench ("We're looking for someone... A young woman..."), and Azrael discloses the divine half shortly after, at Lucifer's own explicit telepathic instruction ("*Say it.*"). Both halves land in the same chapter — yet every downstream document (this synthesis file, `bond_system_reference.md`'s Current Bond Holders list) tracked Elizabeth as Travel-Bond-only through Ch.10, and the working plan for Ch.11 assumed a *new* disclosure would be needed there to justify the roof-jump ability, with Lucifer genuinely uncertain whether it would even work.
