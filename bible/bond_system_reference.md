# THE BOND SYSTEM — Mechanics Reference
...
### Travel Bond Range (established Ch.9 filler scene; refined 2026-09-04)
- **Effective range: approximately 20 feet from a bonded human anchor.** First discovered on-page when Lucifer drifted past it for the first time with Elizabeth as the anchor — neither he nor Azrael knew the limit existed beforehand, and they paced it out empirically against a fence line afterward. Confirmed to be a full 20-foot radius around the anchor (tested from two directions), not a line-of-sight or straight-path trigger.
...
- **Note on the Ch.9 filler's own dialogue:** Tristan's on-page conclusion ("It's not the direction. It's her.") was drawn from incomplete data — Elizabeth was the only Travel Bond in range during that test, so it read as anchor-specific. It isn't a contradiction of the multi-anchor rule above; it's simply what the data looked like with only one bonded person nearby. No prose fix needed.
- **[OPEN] — whether this constraint is active at all times or only under specific conditions.** The scene deliberately leaves this unresolved (neither character fully understands it yet). Flag to the Director before writing a scene that depends on the exact boundary of this rule.
...
## CURRENT BOND HOLDERS (as of Ch.37)
